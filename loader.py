...
class SpeakerLabeledDataset(Dataset):
    def __init__(self, data_folder):
        # data_folderは各話者の音声ファイルが格納されたフォルダのパス
        self.data_folder = data_folder
        self.filepaths = []
        self.labels = []

        # フォルダ内のファイルをロード
        for label, speaker_folder in enumerate(sorted(os.listdir(data_folder))):
            speaker_path = os.path.join(data_folder, speaker_folder)
            for filename in os.listdir(speaker_path):
                if filename.endswith('.wav'):  # ここは使用するファイル形式に合わせて変更
